...
def p_lista_argumentos(p):
    ...
    if len(p) > 2:
        ...
        filho2 = MyNode(name='VIRGULA', type='VIRGULA', parent=pai)
        filho_sym = MyNode(name=',', type='SIMBOLO', parent=filho2)
        p[2] = filho2
...
def p_lista_argumentos_error(p):
    ...
    print("Erro na lista de argumentos. Verifique a sintaxe.")
    error_line = p.lineno(2)
    father = MyNode(name='ERROR::{}'.format(error_line), type='ERROR')
    logging.error("Syntax error parsing argument list at line {}".format(error_line))
    parser.errok()
    p[0] = father
